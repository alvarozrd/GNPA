...
DETAILS_CSV = "detalhes_gnpa_sem_monobit.csv"
SUMMARY_CSV = "resumo_gnpa_sem_monobit.csv"
ENV_CSV = "ambiente_gnpa_sem_monobit.csv"
REPORT_TXT = "relatorio_gnpa_sem_monobit.txt"
...
# =========================================================
# GNPA SEM MONOBIT INTERNO
# =========================================================
class GNPASemMonobit(GNPABase):
    def gerar(self):
        current_seed = self.seed
        x1, x2, x3, x4 = self.produzir_congruencia()
        estado = (current_seed ^ x1) & 0xFFFFFFFF
...
def monobit_pass_rate(numbers):
    passed = sum(1 for n in numbers if monobit_test_32(n))
    return 100.0 * passed / len(numbers)
...
def summarize_group(rows):
    ...
    mono_rates = [r["monobit_pass_rate_percent"] for r in rows]
    ...
    return {
        ...
        "p_min": min(pvals),
        "p_max": max(pvals),
        "monobit_rate_medio": mean(mono_rates),
        "monobit_rate_min": min(mono_rates),
        "monobit_rate_max": max(mono_rates),
        "aprovados": aprovados,
...
def benchmark_generator(name, generator_factory, sample_sizes, base_seeds):
    ...
    for size in sample_sizes:
        ...
        for rep, seed in enumerate(base_seeds, start=1):
            ...
            mono_rate = monobit_pass_rate(numbers)
            ...
            row = {
                ...
                "p_valor": p_value,
                "qualidade": quality,
                "monobit_pass_rate_percent": mono_rate,
        ...
        print(f"Monobit(pass) médio: {summary['monobit_rate_medio']:.2f}%")
        print(f"Faixa Monobit(pass): {summary['monobit_rate_min']:.2f}% - {summary['monobit_rate_max']:.2f}%")
    ...
    with open(filepath, "w", newline="", encoding="utf-8") as f:
        ...
        for r in rows:
            writer.writerow([
                ...
                f"{r['p_valor']:.9f}",
                r["qualidade"],
                f"{r['monobit_pass_rate_percent']:.6f}",
    ...
    with open(filepath, "w", newline="", encoding="utf-8") as f:
        ...
        for (gerador, amostra), group_rows in grouped.items():
            ...
            writer.writerow([
                ...
                f"{s['p_min']:.9f}",
                f"{s['p_max']:.9f}",
                f"{s['monobit_rate_medio']:.6f}",
                f"{s['monobit_rate_min']:.6f}",
                f"{s['monobit_rate_max']:.6f}",
                s["aprovados"],
                s["reprovados"],
...
def write_report(filepath, rows):
    summary_map = build_grouped_summary(rows)

    with open(filepath, "w", encoding="utf-8") as f:
        f.write("RELATÓRIO - GNPA ORIGINAL VS GNPA SEM MONOBIT VS MT\n")
        f.write("=" * 74 + "\n\n")
        f.write(f"Data/hora: {datetime.now().isoformat(timespec='seconds')}\n\n")

        for size in SAMPLE_SIZES:
            f.write(f"Amostra: {format_sample_size(size)}\n")
            for gen in ["GNPA original", "GNPA sem Monobit", "Mersenne Twister"]:
                ...
                f.write(
                    ...
                    f"Monobit médio={s['monobit_rate_medio']:.2f}% | "
...
if __name__ == "__main__":
    ...
    all_results.extend(
        benchmark_generator(
            "GNPA sem Monobit",
            lambda seed: GNPASemMonobit(seed, use_real_time=USE_REAL_TIME),
            SAMPLE_SIZES,
            seeds,
        )
    )
    ...
    write_report(REPORT_TXT, all_results)
